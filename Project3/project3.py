...
for i in range(maxiter):
    ...
    for k, t_k in enumerate(teval):
        ...
        # TODO: Implement R which changes over time
        R = np.diag([t_k*sigma**2])
        W = np.linalg.inv(R)
    ...
    if abs(new_cost - old_cost) / new_cost <= tol:
        break
...
# solution_comp_plot = plt.figure(figsize = (10, 3))
# solution_comp_ax = solution_comp_plot.gca()
# solution_comp_ax.plot(measured_states.t, measured_states.y[0, :], '--', label = 'True Solution')
# solution_comp_ax.plot(glsdc_guess_traj.t, glsdc_guess_traj.y[0, :], label = 'GLSDC Solution')
# solution_comp_ax.set_title('Comparison of True Solution to GLSDC Solution')
# solution_comp_ax.set_xlabel(r'$t$')
# solution_comp_ax.set_xlim(tspan)
# solution_comp_ax.set_ylabel(r'$x(t)$')
# solution_comp_ax.legend()
